/* 
 * Modified QuadTree implementation.
 * 
 * ----------------------
 * 
 * ORIGINAL LICENCE ON : https://bitbucket.org/C3/quadtree
 * 
...
namespace Sparkle.Engine.Base
{
	...
	public class QuadTree<T> : ICollection<T> where T : IQuadStorable
	{
		...
		{
			quadTreeRoot = new QuadTreeNode<T> (new Frame (x, y, width, height));
		}
		...
		public List<T> GetObjects (Frame rect)
        {
            if (rect.X == this.QuadRect.X 
                && rect.Y == this.QuadRect.Y 
                && rect.Width == this.QuadRect.Width 
                && rect.Height == this.QuadRect.Height)
                return this.GetAllObjects();

			return quadTreeRoot.GetObjects (rect);
		}

		public IEnumerable<T> SearchObject (Func<T,bool> predicat)
		{
			return this.GetAllObjects ().Where (predicat);
		}
		...
		public void GetObjects (Frame rect, ref List<T> results)
		{
	...
	public class QuadTreeNode<T> where T : IQuadStorable
	{
		...
		/// </summary>
		/// <param name="rect">The area this QuadTree object will encompass.</param>
		public QuadTreeNode (Frame rect)
		{
			this.rect = rect;
		}
		...
		private QuadTreeNode (QuadTreeNode<T> parent, Frame rect)
			: this (rect)
		{
			this.parent = parent;
		}
		...
		private void Subdivide ()
		{
			...
			Vector2 size = new Vector2 (rect.Width / 2, rect.Height / 2);
			Vector2 mid = new Vector2 (rect.X + size.X, rect.Y + size.Y);

			childTL = new QuadTreeNode<T> (this, new Frame (rect.Left, rect.Top, size.X, size.Y));
			childTR = new QuadTreeNode<T> (this, new Frame (mid.X, rect.Top, size.X, size.Y));
			childBL = new QuadTreeNode<T> (this, new Frame (rect.Left, mid.Y, size.X, size.Y));
			childBR = new QuadTreeNode<T> (this, new Frame (mid.X, mid.Y, size.X, size.Y));
		...
		private QuadTreeNode<T> GetDestinationTree (QuadTreeObject<T> item)
		{
			...
			if (childTL.QuadRect.Contains (item.Data.Bounds)) {
				destTree = childTL;
			} else if (childTR.QuadRect.Contains (item.Data.Bounds)) {
				destTree = childTR;
			} else if (childBL.QuadRect.Contains (item.Data.Bounds)) {
				destTree = childBL;
			} else if (childBR.QuadRect.Contains (item.Data.Bounds)) {
				destTree = childBR;
			}
		...
		internal void Insert (QuadTreeObject<T> item)
		{
			...
			if (!rect.Contains (item.Data.Bounds)) {
		...
		/// <summary>
		/// Get the objects in this tree that intersect with the specified Frame.
		/// </summary>
		/// <param name="searchRect">The Frame to find objects in.</param>
		internal List<T> GetObjects (Frame searchRect)
		{
		...
		/// <param name="results">A reference to a list that will be populated with the results.</param>
		internal void GetObjects (Frame searchRect, ref List<T> results)
		{
			...
			if (results != null) {
				if (searchRect.Contains (this.rect)) {
					...
					if (objects != null) {
						for (int i = 0; i < objects.Count; i++) {
							if (searchRect.Intersects (objects [i].Data.Bounds)) {
